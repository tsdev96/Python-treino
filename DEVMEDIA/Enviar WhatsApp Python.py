...
#find_elements é um métod de navegador(webdriver)
while len(navegador.find_elements_by_id("side")) < 1: #Esperando aparecer o id side(área do código onde ficam as conversas) O Python espera a página carregar a este ponto
    time.sleep(1)#Aguarda 1 segundo antes de seguir

# já estamos com o login feito no whatsapp web
for i, mensagem in enumerate(contatos_df['Mensagem']):#Para cada mensagem na coluna mensagem, guardaremos o valor da coluna em mensagem. enumerate pega a coluna.
    pessoa = contatos_df.loc[i, "Pessoa"] #No i estamos pegando o índice da tabela e com a mensagem estamos pegando a mensagem da tabela em cada passada do for
    ...
    while len(navegador.find_elements_by_id("side")) < 1:
        time.sleep(1)
    navegador.find_element_by_xpath('//*[@id="main"]/footer/div[1]/div/span[2]/div/div[2]/div[1]/div/div[2]').send_keys(Keys.ENTER)#Buscando o xpath da caixa de mensagem para dar enter
    time.sleep(10)
